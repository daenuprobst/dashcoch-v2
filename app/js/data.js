...
async function getOWID() {
  const url =
    'https://raw.githubusercontent.com/daenuprobst/covid19-cases-switzerland/master/owid_international.csv';
  let df = await readCSV(url);
  df = groupBy(df, ['location', 'date'], 'sum');
  return df;
}
...
async function getTesting() {
  const url =
    'https://raw.githubusercontent.com/daenuprobst/covid19-cases-switzerland/master/testing_international.csv';
  let df = await readCSV(url);
  df = groupBy(df, ['location', 'date'], 'sum');
  return df;
}
...
async function getData() {
  ...
  return {
    ...
    mobility: results[8],
    cantonalMobility: results[9],
    excessMortality: results[10],
    owid: results[11],
    testing: results[12],
    lastUpdated: results[13],
  };
}
